...
Release notes are generated by an LLM before publishing. Configure the repository secret `RELEASE_NOTES_API_KEY` (bearer token for an OpenAI-compatible endpoint) to enable this; without it, or if generation fails, GitHub's default generated changelog is used. Optional repository variables: `RELEASE_NOTES_BASE_URL` (default `http://122.193.22.119:8000/v1`) and `RELEASE_NOTES_MODEL` (default `Qwen3.8-27B`). Generation runs with the project's Node.js runtime (`scripts/generate-release-notes.mjs`).
